...
export class RendicionesDirectasComponent implements OnInit {
  ...
  /** Color del badge de estado — mismo criterio que la pestaña "Rendiciones" (REPORT_STATUS_COLORS). */
  reportStatusBadgeClass(r: any): string {
    if (r?.effectivelyClosed) return 'bg-gray-100 text-gray-500';
    switch (String(r?.status || '')) {
      case 'solicited':          return 'bg-purple-100 text-purple-800';
      case 'open':               return 'bg-blue-100 text-blue-800';
      case 'submitted':          return 'bg-yellow-100 text-yellow-800';
      case 'pending_accounting': return 'bg-amber-100 text-amber-800';
      case 'approved':           return 'bg-green-100 text-green-800';
      case 'rejected':           return 'bg-red-100 text-red-800';
      case 'liquidated':
      case 'reimbursed':         return 'bg-emerald-100 text-emerald-800';
